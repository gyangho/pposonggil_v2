...
// //   const otherUserId = 2; // 실제 상대방의 ID로 대체 필요
...
function Chat() {
  ...
  const fetchChatRoomInfo = async () => {//우산 사는 사람, 파는 사람 ID 받아오기
    try {
      const response = await api.get(`http://localhost:8080/api/chatroom/by-chatroom/${chatRoomId}`);
  ...
  const handleConfirmTrade = async () => {
    try {
      ...
      const response = await api.post(`http://localhost:8080/api/trade`, tradeData);
      alert('거래가 확정되었습니다.');
  ...
  const handleReportUser = async () => {//회원 신고
    try {
      ...
      const response = await api.post('http://localhost:8080/api/report', reportData);
      alert('상대를 신고하였습니다.');
      setShowReportModal(false);
    } catch (error) {
      console.error('Failed to report user:', error);
